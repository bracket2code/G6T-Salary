/*
  ...
  1. Nueva Tabla
    - `user_external_data`
      - Almacena todos los datos del JWT decodificado
      - Incluye datos personales, empresariales y relaciones

  2. Nueva Tabla
    - `user_parameter_relations`
      - Almacena las relaciones de parámetros del usuario
      - Incluye tipo, cantidad y referencias
...
CREATE TABLE IF NOT EXISTS user_external_data (
  ...
  external_id text, -- ID del usuario en la API externa
...
CREATE INDEX IF NOT EXISTS idx_user_parameter_relations_type ON user_parameter_relations(relation_type);
