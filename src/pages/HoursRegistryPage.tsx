...
import {
  WorkerInfoModal,
  type WorkerInfoModalState,
  type WorkerInfoData,
  type CompanyParameterOption,
} from "../components/worker/WorkerInfoModal";
import { BulkHoursDistributionModal } from "../features/bulkDistribution/BulkHoursDistributionModal";
import {
  useBulkDistribution,
...
import {
  ...
  ALL_COMPANIES_OPTION_LABEL,
  UNASSIGNED_COMPANY_ID,
  UNASSIGNED_COMPANY_LABEL,
} from "../constants/company";
import { trimToNull, normalizeKeyPart } from "../lib/valueNormalizers";
...
const isLikelyIdentifier = (value?: string | null): boolean => {
  if (!value) {
    return false;
  }
  const trimmed = value.trim();
      ...
      entry.workShifts.forEach((shift, shiftIndex) => {
        ...
        segments.push({
          id:
            ...
            `entry-${
              entry.id ?? "shift"
            }-${entryIndex}-${shiftIndex}-${Math.random()
              .toString(36)
              .slice(2, 6)}`,
          start,
          end,
...
export const HoursRegistryPage: React.FC = () => {
  ...
  const [feedbackDialog, setFeedbackDialog] =
    useState<SaveFeedbackState | null>(null);

  const showFeedbackDialog = useCallback((payload: SaveFeedbackState) => {
    setFeedbackDialog({
      title: payload.title,
  ...
  const bulkDistributionDayKey = bulkDistributionTarget?.day.dateKey ?? null;
  ...
  const openDayHeaderActions = useCallback((target: DayHeaderActionsTarget) => {
    ...
    (target: DayHeaderActionsTarget) => {
      ...
      setSegmentModalTarget({
        ...
        multiAssignments: target.assignments.map((assignment) => {
          const dayData =
            workerWeekData[assignment.workerId]?.days?.[target.day.dateKey];
  ...
  const openWorkerInfoModal = useCallback(
    async (workerId: string, workerName: string) => {
      ...
      const targetWorkerId = existingWorker?.id ?? workerId;
      const targetWorkerName = existingWorker?.name ?? workerName;

      const baseInfo = existingWorker
        ? buildWorkerInfoFromWorker(existingWorker)
        : null;
      const fallbackInfo = baseInfo
        ? await enrichWorkerInfoData(baseInfo)
        : null;

      const missingApiConfigMessage =
        "Falta configuración de API o token para obtener los datos del trabajador.";
      const shouldFetchFromApi = Boolean(apiUrl && externalJwt);
      ...
      setWorkerInfoModal({
        ...
        error:
          !shouldFetchFromApi && !fallbackInfo ? missingApiConfigMessage : null,
        data: fallbackInfo,
      });
      ...
      const endpoint = buildApiEndpoint(
        apiUrl,
        `/Parameter/${encodeURIComponent(targetWorkerId)}`
      );
      ...
      try {
        ...
        const companyAggregates = new Map<
          ...
          { id: string; name: string; count: number }
        >();
        let contractsByCompany: Record<string, WorkerCompanyContract[]> = {};

        const seedCompanyAggregate = (
          rawId?: string | null,
          rawName?: string | null,
          count?: number
        ) => {
          const normalizedId =
            normalizeKeyPart(rawId) ??
            (trimToNull(rawName)
          ...
          return { aggregateKey: normalizedId, displayName };
        };

        const registerContractsForKeys = (
          keys: Array<string | null | undefined>,
          contractList?: WorkerCompanyContract[]
        ) => {
          if (!contractList?.length) {
            return;
          }
          const preferredKey =
            keys.find((key) => key && !isLikelyIdentifier(key)) ??
            keys.find((key) => Boolean(trimToNull(key))) ??
            null;
          if (!preferredKey) {
            return;
          }
          const targetKey = trimToNull(preferredKey) ?? preferredKey;
          const bucket = contractsByCompany[targetKey] ?? [];
          contractList.forEach((entry) => {
            bucket.push({ ...entry });
          });
          contractsByCompany[targetKey] = bucket;
        };

        if (baseInfo?.companies?.length) {
          baseInfo.companies.forEach((company) => {
            const { aggregateKey, displayName } = seedCompanyAggregate(
              company.id,
              company.name,
              company.count
            );
            const baseContracts =
              baseInfo.contractsByCompany?.[company.name] ??
              (company.id
                ? baseInfo.contractsByCompany?.[company.id]
                : undefined);
            registerContractsForKeys(
              [displayName, aggregateKey, company.id, company.name],
              baseContracts
            );
          });
        }
        ...
        if (parameterRelations.length > 0) {
          parameterRelations.forEach((relation, index) => {
            ...
            const effectiveName =
              existingAggregate?.name ?? displayName ?? aggregateKey;
            ...
            if (existingAggregate) {
              ...
              if (
                displayName &&
                (isLikelyIdentifier(existingAggregate.name) ||
                  (!isLikelyIdentifier(displayName) &&
        ...
        const dedupeContractEntries = (
          entries: WorkerCompanyContract[] = []
        ): WorkerCompanyContract[] => {
          const seen = new Set<string>();
          return entries.filter((entry) => {
            const idKey = trimToNull(entry.id)?.toLowerCase() ?? "";
            const nameKey =
              trimToNull(entry.label) ??
              trimToNull(entry.position) ??
              trimToNull(entry.description) ??
              "";
            const rateKey =
              typeof entry.hourlyRate === "number"
                ? entry.hourlyRate.toFixed(4)
                : "";
            const fingerprint = `${idKey}|${nameKey}|${rateKey}`;
        ...
        Object.entries(contractsByCompany).forEach(([rawKey, list]) => {
          ...
          const aggregateEntry = normalizedKey
            ? companyAggregates.get(normalizedKey)
            : undefined;
          const canonicalName =
            aggregateEntry?.name ??
            (!isLikelyIdentifier(rawKey) ? trimToNull(rawKey) : null) ??
            normalizedKey ??
            rawKey;
          const targetName = canonicalName ?? rawKey;
          const bucket = mergedContractsMap.get(targetName) ?? [];
          list?.forEach((entry) => bucket.push({ ...entry }));
          mergedContractsMap.set(targetName, bucket);
        });

        contractsByCompany = Object.fromEntries(
          Array.from(mergedContractsMap.entries()).map(([name, list]) => [
            name,
            dedupeContractEntries(list),
          ])
        );
        ...
        const companyStatsFromApi: Record<string, WorkerCompanyStats> = {
          ...(baseInfo?.companyStats ?? {}),
        };
        companies.forEach((company) => {
          const contractList = contractsByCompany[company.name] ?? [];
          const stats: WorkerCompanyStats = {
            companyId: company.id,
            contractCount: contractList.length,
            assignmentCount: Math.max(
              company.count,
              companyStatsFromApi[company.name]?.assignmentCount ?? 0
            ),
          };
          companyStatsFromApi[company.name] = stats;
        });
        ...
        const infoData: WorkerInfoData = {
          ...
          name:
            ...
            workerName,
          email: email ?? baseInfo?.email ?? undefined,
          secondaryEmail:
            secondaryEmail ?? baseInfo?.secondaryEmail ?? undefined,
          phone: phone ?? baseInfo?.phone ?? undefined,
          companies,
          contractsByCompany,
          role: role ?? baseInfo?.role,
          situation: situationValue ?? baseInfo?.situation ?? null,
          isActive:
            typeof isActiveValue === "boolean"
              ? isActiveValue
              : baseInfo?.isActive,
          department: department ?? baseInfo?.department ?? undefined,
          position: position ?? baseInfo?.position ?? undefined,
          baseSalary: baseSalaryValue ?? baseInfo?.baseSalary,
          hourlyRate: hourlyRateValue ?? baseInfo?.hourlyRate,
          contractType: contractTypeValue ?? baseInfo?.contractType,
          startDate: startDateValue ?? baseInfo?.startDate ?? undefined,
          dni:
            trimToNull(dniValue?.toUpperCase() ?? null) ??
            baseInfo?.dni ??
            undefined,
          socialSecurity:
            trimToNull(socialSecurityValue) ??
            baseInfo?.socialSecurity ??
            undefined,
          birthDate:
          ...
          subcategory:
            trimToNull(subcategoryName) ?? baseInfo?.subcategory ?? undefined,
          subcategoryId:
            trimToNull(subcategoryIdValue) ??
            baseInfo?.subcategoryId ??
            undefined,
          staffType:
            trimToNull(staffTypeValue) ?? baseInfo?.staffType ?? undefined,
        ...
        setWorkerInfoModal((previous) => {
          if (!previous || previous.workerId !== targetWorkerId) {
            return previous;
          }
  ...
  const segmentModalInitialSegments = useMemo(() => {
    ...
    if (segmentModalTarget.mode === "multi") {
      ...
      return (
        segmentModalTarget.multiAssignments?.flatMap((item) => {
          ...
          return extractSegmentsFromEntries(item.existingEntries ?? []).map(
            (segment) => ({
              ...segment,
              assignmentId: item.assignment.id,
            })
    ...
    return extractSegmentsFromEntries(segmentModalTarget.existingEntries ?? []);
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      const { exportHoursRegistryExcel } = await import(
        "../features/hoursRegistry/exportHoursRegistryExcel"
      );

      await exportHoursRegistryExcel({
        ...
        selectedRange,
        rangeLabel,
        hoursComparisonEpsilon: HOURS_COMPARISON_EPSILON,
        dependencies: {
          calculateRowTotal,
  ...
  const renderGroupCard = useCallback(
    (group: GroupView) => {
      ...
      const distributionViewMode: DistributionViewMode | null =
  ...
  return (
    <>
      ...
      <BulkHoursDistributionModal
        isOpen={bulkDistributionTarget !== null}
        targetDay={bulkDistributionTarget?.day ?? null}
        groupName={bulkDistributionTarget?.groupName ?? ""}
        ...
        onClose={closeBulkDistributionModal}
        onApply={handleBulkDistributionApply}
      />
